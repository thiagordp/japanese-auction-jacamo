...
public class AuctionArtifact extends Artifact {
	...
	@OPERATION
	public void start(String task, Double price) {
	...
	public void bid() {
		...
		// if (bidValue < opCurrentValue.doubleValue()) { // the bid is better than the
		// previous
		// opCurrentValue.updateValue(bidValue);
		// currentWinner = getCurrentOpAgentId().getAgentName(); // the name of the
		// agent doing this operation
		// }

		System.out.println("Received bid  from " + getCurrentOpAgentId().getAgentName() + " for "
				+ getObsProperty("task").stringValue());
